...
configuration_directory = os.path.join(os.path.expanduser("~"), 'Library',
                                       'Containers', app_uri)
...
def usage(script_name):
    ...
    print()
    print(
        'create iTerm windows & tabs with set title and start / stop command')
    print()
    print('Commands:')
...
async def iTermStart(connection):
    ...
    if config['windows'] is None:
        config['windows'] = {}
    config['windows'][new_window.window_id] = {
        'tabs': [new_window.current_tab.tab_id]
    }
    ...
    for tabTitle in tabs:
        ...
        if firstTab is True:
            ...
            config['windows'][new_window.window_id]['tabs'].append(
                new_tab.tab_id)
    ...
    await current_window.current_tab.current_session.async_inject(
        'created {} window with {} tabs\n'.format(1, len(tabs)).encode())
...
def init(args):
    ...
    if os.path.exists(configuration_file) and os.path.isfile(
            configuration_file):
